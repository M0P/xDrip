...
public class Cache {
    ...
    private static String checkPath(final String path, final PluginDef def, final boolean erase) {
        try {
            final File f = new File(path, def.name + ".dex");
            if (f.exists()) {
                if (erase) {
                    f.delete();
                    return null;
                }
                final File fs = new File(path, def.name + ".sig");
                if (fs.exists()) {
                    final File fv = new File(path, def.name + ".ver");
                    if (fv.exists()) {
                        final String v1 = new String(readFromFile(TAG, fv));
                        if (def.version.equals(v1)) {
                            final byte[] b1 = readFromFile(TAG, f);
                            final byte[] b2 = readFromFile(TAG, fs);
    ...
    private static String[] getPaths() {
        final String appDir = xdrip.getAppContext().getFilesDir().getPath();
        return new String[]{appDir};
    }

    public static String getPath(final PluginDef def) {
        for (final String c : getPaths()) {
            final String r = checkPath(c, def, false);
            if (r != null) return r;
        }
        return null;
    }
