...
public class DisplayQRCode extends BaseAppCompatActivity {
    ...
    public boolean generateKeksBinaryPrefs() {
        ...
        try {
            ...
            final String tem = "keks_p";
            for (int i = 1; i < 4; i++) {
                final String pn = tem + i;
                final byte[] bb = JoH.hexStringToByteArray(Pref.getStringDefaultBlank(pn));
                if (bb == null || bb.length == 0) {
                    Log.d(TAG, "Null or empty at: " + i);
                    return false;
                }
                final String px = "b__" + pn;
...
//        showQrCodeFromBinaryPrefsMap("G Key settings\n\n" + Preferences.getMapKeysString(binaryPrefsMap).replace("\n", " ") + "\n\nHash: " + mapChecksum.substring(0, 16));
        showQrCodeFromBinaryPrefsMap(
                "G Key settings\n\n"
                        + Preferences.getMapKeysString(binaryPrefsMap).toString().replace("\n", " ")
                        + "\n\nHash: " + mapChecksum.substring(0, 16)
        );
    }
...
//            uploadBytes(result, 1);
    ...
    private void showQRCode() {
        final String mystring = new JSONObject(prefsMap).toString();
        Log.d(TAG, "Serialized: " + mystring);
        final String compressedstring = JoH.compressString(mystring);
    ...
    private void showQrCodeFromBinaryPrefsMap(final String hint) {
        final byte[] bytes = serializeBinaryPrefsMap(binaryPrefsMap);
        // show binary type
        Log.d(TAG, "QR bytes: " + bytes.length);
        final byte[] bytesc = JoH.compressBytesToBytes(bytes);
    ...
    private void showQRCode2(byte[] bytesc, String prefix, final String hint) {
        ...
        final double scale = (getScreenWidth() > getScreenHeight()) ? 0.8d : 1;
        final int desiredPixels = (int) (Math.min(getScreenWidth(), getScreenHeight()) * scale);
        try {
            final Bitmap bitmap = createQRCodeBitmap(bytesc, desiredPixels, desiredPixels, prefix);
